...
import { MEDIA_CONFIG, piscinaMaxThreads, THUMBNAIL_CONFIG } from '@/lib/config';
...
const piscina = new Piscina({
    filename: path.resolve(process.cwd(), 'lib/heic-worker.js'),
    maxThreads: os.cpus().length - 1, // 最大线程数
...
export async function heic2Jpeg(fullPath, buffer) {
    ...
    if (fp.endsWith('.heic') || fp.endsWith('.heif')) {
        try {
            return await piscina.run(buffer);
        } catch (err) {
            // 如果转码失败（比如 HEIC 文件损坏），捕获异常，退回原图
            console.error(`[HEIC 转码失败] ${fullPath}:`, err.message);
            return buffer;
        }
    }
    return buffer;
