...
function runHeicConvertWorker(buffer) {
    return new Promise((resolve, reject) => {
        ...
        const worker = new Worker(`
            ...
            (async () => {
                try {
                    const inputBuffer = Buffer.from(workerData.buffer);
        ...
        worker.once('exit', (code) => {
            if (code !== 0) {
                reject(new Error(`HEIC convert worker stopped with exit code ${code}`));
            }
        });
    });
}
...
export async function heic2Jpeg(fullPath, buffer) {
    if (isHeicFile(fullPath)) {
        return await runHeicConvertWorker(buffer);
    }
    return buffer;
}
